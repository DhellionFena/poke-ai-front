  ...
  return (
    <div className="grid grid-cols-2 w-full h-full border border-black gap-4 p-4 rounded-lg shadow-lg overflow-auto bg-zinc-200" >
      {moveset.map((move, index) => (
        <MoveCard key={index} statusAtaques={statusAtaques.find((value) => value.nomeAtaque === move.nome)} ataqueEscolhido={ataqueEscolhido} escolherAtaque={escolherAtaque} move={move} />
      ))}
    </div>
  );
}
